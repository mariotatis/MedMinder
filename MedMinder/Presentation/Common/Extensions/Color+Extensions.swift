...
extension Color {
    static let background = Color(UIColor { traitCollection in
        return traitCollection.userInterfaceStyle == .dark ? UIColor(hex: "121212") : UIColor.systemGroupedBackground
    })
    ...
    static let textSecondary = Color(UIColor { traitCollection in
        return traitCollection.userInterfaceStyle == .dark ? .gray : .systemGray
    })
    
    // Custom Initializer for Hex
    init(hex: String) {
